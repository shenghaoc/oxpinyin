...
## Addendum 5 — item 5 (J internal coverage, 2026-09-26T02:45:01Z UTC)

This addendum precedes all item-5 measurements. Item 5 is static: no source
or subject builds, no configuration run, no linked test harness. Read only
the pinned upstream source at `074a2219`, the audited Rust source at
`18d78208`, and already-built oracle/subject objects. The 131 C exports
remain the roots, including libzhuyin.

**Extraction.** A scratch script generates a TSV ledger and a separate edge
ledger. Enumerate direct function calls using the pin's source AST (a
`clang++ -fsyntax-only` parse with no code generation) and cross-check with
the existing oracle objects' `nm`/`objdump` and DWARF source lines. Expand
transitively from every export. Template instantiations, static functions,
constructors, destructors, callbacks and virtual targets are separate nodes
where the compiler/source distinguishes them. Record every unresolved
indirect call or parse failure as such; do not silently omit its possible
targets. The report gives the exact regeneration command and counts of
roots, reachable nodes, mapped nodes, deliberately absent nodes, unresolved
edges and unparsed translation units. It also gives the command that
regenerates the 79+52 export set independently of the ledger.

**Counterpart rule.** Each reachable upstream function gets the upstream
file and line, a Rust counterpart `path:line` with a source-checked semantic
role, or `ABSENT` with a cited reason. Name resemblance alone is not a
mapping. A function with no verified counterpart or cited absence is
`UNMAPPED`, never silently counted as covered. A conservative over-approximate
graph may include extra nodes, but cannot justify COMPLETE until its excess
is distinguished from reachable nodes.

**Verdict.** The export set may be static MATCH if the independently
regenerated symbol sets agree. The internal ledger is COMPLETE only if all
reachable functions are mapped or cited ABSENT, the graph has no unresolved
call target or unparsed translation unit, the independent symbol/source
cross-check agrees, and both mutations below are detected and reverted.
It is PARTIAL with exact unmapped/unresolved counts when a useful ledger
exists but any condition fails. It is NOT-ESTABLISHED if no reproducible
graph/ledger can be produced. No internal row is called MATCH on the basis
of ABI symbols alone.

**Mutations.** `J-m1` deletes one export root from a scratch copy of the
ledger; the checker must detect the missing root and pass again after
restoration. `J-m2` deletes one transitively reachable non-export function
row from the scratch ledger; the checker must detect it and pass after
restoration. Neither mutation touches the repo worktree or pinned source.

**Falsifiers.** A missing export, an unresolved function-pointer/virtual
target, a compile-unit parse failure, an inline/callback edge absent from
the binary-only graph, a claimed counterpart that does not implement the
same role, or a count not regenerated by the published command invalidates
COMPLETE. If either injected deletion is not caught, downgrade the internal
ledger to NOT-ESTABLISHED; do not substitute another mutation after seeing
the result.

## Addendum 6 — item 6 (L complexity, 2026-09-26T03:22:50Z UTC)

This supersedes the wall-time/RSS instrument and threshold in the original L
section above. No L measurement preceded this addendum. The subject remains
the audited `18d78208` and each oracle uses its matched backend. The source
inventory of internal structural divergences is enumerated independently of
the partial J name-level candidate ledger; a missing item makes L incomplete.

**Instrument and workloads.** For each source-identified structural
divergence, use the same C driver, input and data directory on both libraries
in each cell. Measure whole-process executed instructions (`Ir`) with
`valgrind --tool=callgrind --cache-sim=no --branch-sim=no`, and peak live heap
with `valgrind --tool=massif --time-unit=i --heap=yes --stacks=no` (or a
validated peak-live-heap allocator counter). Include the §12 guess/sentence
set, 200-input training, 10k-phrase import/export and cold open, or explicitly
mark an unavailable workload NOT-ESTABLISHED. Capture `/proc/loadavg` and
`date -u +%FT%TZ` immediately before each run, even though Ir and live heap
are not wall-clock measures. No wall-clock result is used while other lanes
build; if one is reported, attach its concurrent load. Preserve raw
Callgrind/Massif files and driver-output digests in scratch. Use three repeats
per side/workload/cell and the median Ir and median peak heap; report spread.

**Decision rule.** The pre-existing 1.10 threshold applies to median Ir and
median peak live heap in place of time and RSS. Both subject/oracle ratios
strictly above 1.10 on the same comparable workload/cell, outside the
three-run spread, means BOTH-WORSENED and a divergent complexity finding.
One ratio alone is a measured trade, not a pass over all structure. An
unmeasured structure, noncomparable driver output, missing cell, missing
asymptotic source justification, or unresolved J call target makes L
NOT-ESTABLISHED for that scope. Complete coverage requires every identified
structural divergence and both dimensions in all three cells. A source
asymptotic argument is cited separately from the empirical ratios.

**Mutations.** In scratch copies only, L-m1 inserts the already specified
input-length-squared busy loop in `pinyin_guess_candidates`; the same
Callgrind instrument must detect higher Ir and return to baseline after
revert. L-m2 retains 64 MiB from `pinyin_init`; Massif must detect higher
peak live heap and return to baseline after revert. Run each on all three
cells. No source-repository file changes or fix commits.

**Falsifiers.** A missing `Ir` event, Massif's peak not from live heap,
different data/driver paths, output mismatch that makes work unequal,
unrecorded load, >10% pristine repeat spread, undetected mutation, or
non-reverting baseline downgrades that workload/cell to NOT-ESTABLISHED.
No wall-clock substitute is allowed merely because instrumentation is slow.
