...
pub use pinyin_user::{ExportedPhrase, USER_DICTIONARY};
...
///
/// Returns `None` for a null context, a context without a user store, or a
/// store read failure; otherwise the same §9 rows
/// [`iterators::pinyin_begin_get_phrases`] materializes. This is a safe Rust
/// convenience over the same state, not a new C ABI symbol.
#[must_use]
pub fn user_phrase_rows(context: *mut PinyinContext) -> Option<Vec<ExportedPhrase>> {
    state::export_user_phrase_rows(context)
}
