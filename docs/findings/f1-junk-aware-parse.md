...
| `replace_raw` | every character (closing class B2; lands with `fix/parse-termination`) | capi `pinyin_parse_more_*` seam; keeps the bytes the pin stops at |
...
**Addendum (lands with `fix/parse-termination`):** `replace_raw`
originally shared the `type_pinyin` filter, which silently dropped space, control,
