...
int main(int argc, char **argv) {
    ...
    /* DB_NOTFOUND is the walk's normal end; anything else is a cursor
     * failure the counters below must not paper over. */
    int cursor_failed = rc != DB_NOTFOUND;
    if (cursor_failed)
        fprintf(stderr, "c_get: %s\n", db_strerror(rc));
    cur->c_close(cur);
    db->close(db, 0);
    ...
    /* The exit status is what the sanitizer gate reads: a nonzero
     * violation counter — or a failed cursor — is a failure, never a
     * clean print. */
    if (cursor_failed || keysize_bad || bad_size || unsorted ||
        total_mismatch || zero_item_nonzero_total) {
        printf("VERDICT: FAIL\n");
        return 1;
    }
    printf("VERDICT: ok\n");
