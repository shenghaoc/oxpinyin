...
//! key, byte for byte.
//!
//! Pure byte manipulation with no Berkeley DB involvement, so it is
//! unit-testable without libdb and is where the format is stated once.
//!
...
//! 56,359 records and 1,849,609 successor items, every value `4 + 8n`
//! bytes, every item array strictly ascending, and every `total_freq`
//! equal to the sum of its items' frequencies. Those two counts are also
//! what `oxpinyin-datagen` derives independently from `model20`
//! (`docs/findings/datagen-model20.md`), which is a second, unrelated
//! confirmation that this reading of the layout is the right one.
//!
...
mod tests {
    ...
    fn encode_reproduces_the_pins_own_bytes() {
        ...
        // from the probe: prev 0x03001801, total 65, four items.
        let mut gram = SingleGram::new();
        gram.set_total(65);
