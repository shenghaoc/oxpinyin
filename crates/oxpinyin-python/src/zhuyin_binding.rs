...
//! [`crate::zhuyin::ZhuyinSession`] (Rust structs ↔ Python objects,
//! `EngineError` → exceptions), never behaviour. The shared
//! [`oxpinyin_facade`] orchestration plus the session's snapshot driver
//! own the zhuyin law; this layer only shapes it for Python, reusing the
//! crate's shared translators (`open_error`, `engine_error`,
//! `with_locked`, `kind_label`) so the two engines report identical
...
use crate::zhuyin::{
    ZhuyinCandidate as SnapshotCandidate, ZhuyinSession, chewing_scheme_from_value,
...
impl ZhuyinEngine {
    fn open_with(system_dir: PathBuf, user_dir: Option<PathBuf>) -> PyResult<Self> {
        ...
        Ok(Self {
            inner: Arc::new(Mutex::new(ZhuyinSession::open(&runtime, session))),
        })
    }
}
...
fn convert(candidate: &SnapshotCandidate) -> ZhuyinCandidate {
    ZhuyinCandidate {
...
fn snapshot(candidates: &[SnapshotCandidate]) -> Vec<ZhuyinCandidate> {
