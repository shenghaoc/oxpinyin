...
79/79** `pinyin_*` symbols, so the produced `.so` is functionally
indistinguishable from libpinyin regardless of caller. (`pinyin.h`'s
`#if 0`-declared `pinyin_get_raw_full_pinyin` stays excluded — it has no
...
implemented live, with no placeholder bodies.
