//! Runtime rpath for this package's Kyoto Cabinet producer.
//!
...
//! rpath for the binary this package links, mirroring the store's
//! discovery exactly; with the `kyotocabinet` feature off it emits
//! nothing. As in the store, the rpath is a convenience, not the
//! contract: an artifact built without it must still find the library
