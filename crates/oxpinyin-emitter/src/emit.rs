...
//! same one `oxpinyin_data::parse_interpolation2` reads for the `\1-gram`
//! section (`crates/oxpinyin-data/src/interp.rs`). Counts only — no λ
...
use oxpinyin_segment::{PhraseLexicon, SENTENCE_START};
...
mod tests {
    ...
    fn parse_interpolation_dump_round_trips_values() {
        let (counts, lexicon) = sample();
        let text = emit_interpolation2(&counts, &lexicon);
        let parsed = oxpinyin_counter::parse_interpolation_dump(&text);
