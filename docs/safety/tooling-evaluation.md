...
- Reachable valuable targets: `oxpinyin-store` under
  `--no-default-features --features redb` (the pure-Rust peer backend)
  tests; the safe-core fuzz corpus replayed under Miri (parser/scheme/
  codec are pure functions — ideal); `oxpinyin-core` graph/k-best
  invariants. Miri cannot see the KC / Tkrzw / LMDB C sides — those are
  first-class peer backends here too, and their integrity is covered by
...
project config. **Verdict: DROP — was ADOPT (one committed settings file, explicitly
not CI-enforced); superseded by §22 and dropped in review.**
