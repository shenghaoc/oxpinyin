...
### interpolation2.text (runtime input)

`interpolation2.text` is not generated by datagen — it is an input
sourced from the pinned model20 archive
(SHA-256 `59c68e89d43ff85f5a309489499cbcde282d2b04bd91888734884b7defcb1155`,
verified by `tools/model/fetch-model.sh`). Both datagen runs used the
same cached copy extracted from that archive, so the file is identical
across runs by construction. All four runtime files (three `.kct` tables
plus `interpolation2.text`) are therefore covered by the reproducibility
claim.
