...
# (fixtures/w3/<kct|tkt|db>), a real drop-in dir with libpinyin's
...
# this gate builds the default (Berkeley DB since 2026-09-20) and picks
# the first committed backend directory, preferring the default's (db).
if [ -n "${OXPINYIN_CAPI_BACKEND_EXT:-}" ]; then
    ...
    SYS_EXT=""
    # `db` first: the default build is Berkeley DB since 2026-09-20, and
    # all three flavours name their tables identically, so picking the
    # wrong one would open as nonsense rather than fail as a missing file.
