...
impl Engine {
    ...
    /// profile in libpinyin's own file shapes (`user_bigram.db`, the
    /// `.bin` DBM tables, `user.bin`, the `.dbin` logs, `user.conf` —
    /// in the compiled-in backend's container) and enables learning.
    #[new]
