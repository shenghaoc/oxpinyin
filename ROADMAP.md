...
| **1** | Exact-output parity with the pin-built libpinyin oracle (differential testing) — candidate surface bit-identical on all 10,190 corpus rows (`docs/testing/corpus-tail.md`); full exported ABI and user-store surfaces still carry open residues (`docs/findings/probe-coverage-abi.md` B/C as rows 33–34, register row 32, and residue A — the missing user-phrase tail, classification pending); the sentence-trellis float band stays the frozen exception (row 11) |
