...
value must come from a sweep measured against the §4 gates (the six
...
2. **Unigram scale / tie-break for same-length phrases — `UNIGRAM_TIEBREAK_SCALE`
   (16, `crates/oxpinyin-data/src/lm/mod.rs`, empty-history branch only).** F2 swept
...
4. **Model-term strength — `ScoringConfig::lm_weight` and λ.** Both
   provisional (`scoring-spec.md`; λ = `Lambda::PINNED` (`0.312699` from `table.conf`;
   was provisional `1/2` before the 2026-08-15 fix) in
   `crates/oxpinyin-data/src/lm/mod.rs`). Any sweep is global and must clear the
   same gates.
