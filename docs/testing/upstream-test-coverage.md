...
| `test_zhuyin.cpp` | zhuyin pipeline: `parse_more_chewings` → `guess_sentence` → `get_sentence` → train/reset/save/mask-out | ADAPT | `crates/oxpinyin-capi/tests/abi/pipeline.rs` (`the_zhuyin_pipeline…`): oxpinyin unifies the separate `zhuyin_*` C API onto the same instance surface, so the port drives the chewing parser and asserts a sentence answers. The parse half is additionally a frozen pin: `crates/oxpinyin-core/tests/zhuyin_parse_parity.rs` replays `fixtures/w4/oracle-zhuyin-parse.txt` — the test's stdin-line battery shape captured from the pin oracle (`tools/capture/run-capture.sh` family ZH) across all 8 parseable keyboards and the three option words |
