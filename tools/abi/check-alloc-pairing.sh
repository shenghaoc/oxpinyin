...
prepare_fixtures() {
	...
	if [[ -n ${OXPINYIN_CAPI_BACKEND_EXT:-} ]]; then
		...
		kct | tkt | db) ext=$OXPINYIN_CAPI_BACKEND_EXT ;;
		*)
			echo "fatal: OXPINYIN_CAPI_BACKEND_EXT='$OXPINYIN_CAPI_BACKEND_EXT' is not one of: kct tkt db" >&2
			exit 2
			;;
		esac
	else
		local candidate
		for candidate in db tkt kct; do
