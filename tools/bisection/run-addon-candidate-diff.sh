...
SYS="$(mktemp -d)"
trap 'rm -rf "$SYS"' EXIT
cp "$REPO_ROOT/fixtures/w3/pinyin_index".* "$SYS/"
cp "$REPO_ROOT/fixtures/w3/phrase_index".* "$SYS/"
cp "$REPO_ROOT/fixtures/w3/bigram".* "$SYS/"
cp "$REPO_ROOT/fixtures/w3/addon_4_pinyin_index".* "$SYS/"
cp "$REPO_ROOT/fixtures/w3/addon_4_phrase_index".* "$SYS/"
