...
use crate::zhuyin::{
    ZhuyinFacade, chewing_scheme_from_value, dvorak_scheme_message, full_scheme_from_value,
...
///
/// The zhuyin twin of [`run_corpus`]: a fresh [`ZhuyinFacade`] per case —
/// the same facade type the binding wraps — replayed through the
...
pub fn run_zhuyin_corpus(corpus: &Value, system_dir: &Path) -> Result<Value, RunError> {
        ...
        .ok_or_else(|| RunError("corpus has no cases array".to_owned()))?
    {
        ...
        let session = runtime
            ...
            .map_err(|e| RunError(e.to_string()))?;
        let mut facade = ZhuyinFacade::wrap(&runtime, session);
...
/// binding.
fn run_zhuyin_case(facade: &mut ZhuyinFacade, case: &Value) -> Result<Value, RunError> {
    let name = case["name"].as_str().unwrap_or_default().to_owned();
    let mut events = Vec::new();
...
/// API returned.
fn parse_event(facade: &mut ZhuyinFacade, kind: &str, mode: &str, input: &str) -> Value {
...
fn sentence_event(facade: &ZhuyinFacade, kind: &str, ran: bool) -> Value {
    let sentences: Vec<String> = (0..=u8::MAX)
...
fn zhuyin_top_texts(facade: &ZhuyinFacade) -> Vec<String> {
    facade
        .candidates()
        .iter()
        .map(|c| c.text().to_owned())
