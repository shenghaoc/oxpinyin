...
| F-6 | low | capi P1/P2 + `ffi.rs:43` | opaque-handle trust surface (stale/double-free handle = UB) and `g_free`≡libc-`free` pairing assumption (true on glibc, would break under custom GLib allocator) | FFI contract | **PARTLY GATED** (2026-09-09: `tools/abi/check-alloc-pairing.sh` classifies every ABI slot and exercises each one's declared deallocator under ASan/LSan against the GLib actually linked, so the pairing assumption and the handle/destructor half are now enforced per PR — `docs/findings/abi-allocator-pairing.md`. A *stale* or double-passed handle from the consumer stays UB and stays a documented residual: the gate holds the library's half, not the caller's.) |
