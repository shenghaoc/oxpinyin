...
/// order.
#[must_use]
pub fn encode_log_records(records: &[LogRecord]) -> Vec<u8> {
    let mut out = Vec::new();
    ...
    for record in records {
        match record {
            LogRecord::Add { token, new_item } => {
                ...
                out.extend_from_slice(&(new_item.len() as u16).to_le_bytes());
                out.extend_from_slice(new_item);
            }
            LogRecord::Remove { token, old_item } => {
                push_head(2, *token, &mut out);
                out.extend_from_slice(&(old_item.len() as u16).to_le_bytes());
...
mod tests {
    ...
    fn log_records_round_trip_through_the_pin_layout() {
        ...
        let bytes = encode_log_records(&records);
