...
| oxpinyin-store | ordered byte-KV seam; Tkrzw (default since 2026-09-05), Kyoto Cabinet, Berkeley DB (2026-09-12) backends — one per binary, compile-time selected; all are system C-library deps, Linux-verified and macOS-buildable via Homebrew (the CI portable lanes install each OS's backend; the peer matrix stays on Linux) | deny | yes | via engine |
...
(`StoragePaths`); no distro layout is auto-detected.
