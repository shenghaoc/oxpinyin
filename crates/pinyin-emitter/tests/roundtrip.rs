...
//!
//! `pinyin_data::parse_interpolation2` is the decoder's consumer of
...
use pinyin_emitter::{emit_interpolation2, write_interpolation2};
...
fn temp_path(name: &str) -> PathBuf {
    ...
    path.push(format!(
        "pinyin-emitter-roundtrip-{name}-{}.text",
        std::process::id()
    ));
    path
}
...
fn emit_roundtrips_through_parse_interpolation2() {
    ...
    let parsed = pinyin_counter::parse_interpolation_dump(&emit_interpolation2(&counts, &lexicon));
