...
//! held-out bigram **values** equal what `pinyin_counter::Counter` produces
...
use pinyin_counter::parse_ngseg;
use pinyin_segment::{NULL_TOKEN, SENTENCE_START};
...
mod tests {
    ...
    fn matches_counter_bigrams() {
        ...
        let counter = pinyin_counter::Counter::new(true, std::iter::empty()).count(&tokens);
