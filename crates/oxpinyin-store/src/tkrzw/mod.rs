...
//
// A tkrzw-built libpinyin's `punct.bin` is a TreeDBM
// (`punct_table_tkrzwdb.cpp:70`: `m_db = new TreeDBM` in `attach`),
// keyed by the four native-endian bytes of a `phrase_token_t` and
// valued by the raw `PunctTableEntry::m_chunk` content — the same
// backend-independent chunk format the Kyoto Cabinet file carries, so
// only the open mode differs from the bigram reader above.

/// The visitor [`TkrzwPunctDb::for_each`] drives: one `(token, chunk)`
/// row, the chunk bytes undecoded.
pub type TkrzwPunctRow<'a> = dyn FnMut(u32, &[u8]) -> Result<(), StoreError> + 'a;

/// Read-only access to a libpinyin `punct.bin` written by a tkrzw-built
/// libpinyin (TreeDBM).
pub struct TkrzwPunctDb {
    db: Db,
}

impl TkrzwPunctDb {
    /// Opens `path` as a TreeDBM, read-only; fails when the file is
    /// missing rather than creating it.
    ///
    /// # Errors
    ///
    /// [`StoreError`] when the file cannot be opened as a TreeDBM.
    pub fn open_read_only(path: &Path) -> Result<Self, StoreError> {
        validate_path(path)?;
        let path = CString::new(path.as_os_str().as_encoded_bytes())
            .map_err(|_| StoreError::InvalidInput("path contains NUL"))?;
        let params = c"dbm=tree,no_create=true";
        // SAFETY: both strings outlive the call; a NULL handle is the
        // documented failure, taken as an error immediately.
        let db = unsafe { ffi::tkrzw_dbm_open(path.as_ptr(), false, params.as_ptr()) };
        let Some(db) = NonNull::new(db) else {
    ...
    /// Visits every `(token, chunk)` record in key order, the chunk
    /// bytes undecoded.
    ///
    /// # Errors
    ///
    /// [`StoreError`] on a backend failure, a malformed key, or an
    /// error returned by `visit`.
    pub fn for_each(&self, visit: &mut TkrzwPunctRow<'_>) -> Result<(), StoreError> {
        // A TreeDBM walk positions with the iterator's first-record jump
        // (its order is the tree's, and the punct keys carry no frame),
        // mirroring the bigram walk's mechanics with raw values.
        let mut closure = |key: &[u8], value: &[u8]| {
            let bytes: [u8; 4] = key.try_into().map_err(|_| {
                StoreError::Backend(
                    format!(
        ...
        check(unsafe { ffi::tkrzw_dbm_iter_first(iter.0.as_ptr()) })?;
        loop {
            // SAFETY: as in the bigram walk — the callback argument
            // outlives the call and `walk_row` honours the contract.
            let ok = unsafe {
                ffi::tkrzw_dbm_iter_process(
                    iter.0.as_ptr(),
                    Some(walk_row),
...
mod tests {
    ...
    /// Builds a `punct.bin` in the pin's own physical layout — a
    /// TreeDBM with raw 4-byte token keys and raw chunk values —
    /// through the module's own batch writer, then reads it back
    /// through the public reader.
    #[test]
    fn punct_db_walks_a_pin_layout_tree_dbm() {
        let dir = std::env::temp_dir().join(format!("oxpinyin-tkz-punct-{}", std::process::id()));
...

        {
            ...
            // SAFETY: the path string outlives the call; a NULL handle
            // is the documented failure.
            let db = unsafe { ffi::tkrzw_dbm_open(path_c.as_ptr(), true, c"dbm=tree".as_ptr()) };
            let Some(db) = NonNull::new(db) else {
                panic!("tkrzw_dbm_open failed: {}", status_error());
            };
            let db = Db(db);
            ...
            db_apply(
                ...
                &[
                    mutation(7, &comma),
                    mutation(42, &period_comma),
                    mutation(0x0100_0001, &[]),
                ],
            )
            .unwrap();
        }

        let db = TkrzwPunctDb::open_read_only(&path).unwrap();
