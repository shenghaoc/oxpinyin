/*
 * union-probe-diff.c — whole-surface probe over the 58-symbol consumer
 * union (compatibility-policy §(e): every union symbol needs a probe
 ...
 *
 * The driver resolves exactly the 58 union symbols and refuses to run
 * when any one of them is missing from the library, so a run also
...
#undef LOAD
    ...
    for (unsigned fi = 0; fi < 2; fi++) {
        ...
        printf("guess_sentence(%s)=%s\n", in, yesno(s.guess_sentence(inst)));
        char *sent = NULL;
        bool sok = s.get_sentence(inst, 0, &sent); /* proved: row 0 */
