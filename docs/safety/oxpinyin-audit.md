...
| F-7 | low | `iterators.rs:175,298,424`, `cursor.rs:25,56` | five entry points without `ffi_catch` (three iterator-`end` that call `mark_modified()` — a panic there would cross the ABI as abort; two trivial writers) | FFI hygiene, small fix | **FIXED** (8fff932: ffi_catch on all three iterator-ends) |
