...
- **Fixture scale.** `fixtures/w3` is a 2.2 MB mini fixture in fixture mode.
  ...
  is the *ratio* between shared and private for this fixture alone: similar
  between the two interpreters at each thread count (1.00x/1.01x at one
  thread through 4.13x/3.98x at eight), rising with thread count as the
  private column scales while the shared column stays pinned by its mutex.
  Nothing here measures how the ratio moves with data size or a production
  model.
