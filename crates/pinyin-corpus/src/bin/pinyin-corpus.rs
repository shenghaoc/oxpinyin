...
//! `pinyin-segment`.
//!
//! Usage: pinyin-corpus [OPTIONS] [inputfile]
//!
...
//! only — the exact input format `pinyin-segment` reads.
...
use pinyin_corpus::{Converter, Options, clean};
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut config = pinyin_corpus::DEFAULT_CONFIG.to_owned();
...
fn print_help() {
    print!(
        "Usage: pinyin-corpus [OPTIONS] [inputfile]\n\
         ...
         pinyin-segment. Input must be decompressed XML; '-' or no argument\n\
         reads stdin.\n\
         \n\
