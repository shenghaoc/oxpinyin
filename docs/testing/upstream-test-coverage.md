...
| `storage/test_table_info.cpp` | `SystemTableInfo2` load, λ, per-index table infos; `UserTableInfo` conform | ADAPT | λ pinned by `crates/oxpinyin-data/src/table_conf.rs` (`PINNED_LAMBDA`) and `lm/tests.rs` (`default_lambda_is_the_pinned_config_value`); table manifests by `oxpinyin-datagen` (`tests/fixtures_identity.rs`). `UserTableInfo` conform is N/A: the user store has no format-version row, matching libpinyin's unversioned user files (ruling 2026-09-08, `docs/findings/user-store.md` §4), so there is nothing to conform; hostile files are a typed error (`crates/oxpinyin-user/tests/user_store_acid.rs`) |
