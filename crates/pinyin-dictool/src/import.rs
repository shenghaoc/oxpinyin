...
use pinyin_capi::{
    ExportedPhrase, ImportIterator, PinyinContext, add_user_import_phrase, begin_user_import,
    close_user_import_context, end_user_import, open_user_import_context, save_user_import_context,
    user_phrase_rows,
};
...
pub fn run(user_dir: &Path, path: &Path) -> Result<(), ImportError> {
    ...
    for (index, record) in records.iter().enumerate() {
        let current = existing
            .get(&(record.phrase.clone(), record.pinyin.clone()))
            .copied()
            .unwrap_or(0);
        let delta = record.count.saturating_sub(current);
        if delta == 0 {
            continue;
        }
        ...
        let delta_c = c_int::try_from(delta).map_err(|_| ImportError::Add { line: index + 1 })?;
        if !add_user_import_phrase(iter, &record.phrase, &record.pinyin, delta_c) {
            first_error.get_or_insert(index + 1);
        }
    }
...
mod tests {
    ...
    fn temp_dir(tag: &str) -> PathBuf {
        let path =
            std::env::temp_dir().join(format!("pinyin-dictool-{tag}-{}.d", std::process::id()));
    ...
    fn import_round_trips_through_export_and_is_idempotent() {
        ...
        // Export → text → import → export is bit-identical: the exact
        // round-trip the standalone migration flow promises.
        let export_path = dir.join("export.tsv");
        let export_text: String = first
            .iter()
            .map(|row| format!("{}\t{}\t{}\n", row.text, row.pinyin, row.count))
            .collect();
        write(&export_path, &export_text);
        let fresh_user = dir.join("fresh-user");
        run(&fresh_user, &export_path).expect("export import");
        let context = open_user_import_context(&fresh_user);
        assert!(!context.is_null());
        assert_eq!(exported(context), first);
        close_user_import_context(context);
    }
