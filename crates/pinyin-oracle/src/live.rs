...
impl Oracle {
    /// Exports every `(phrase, pinyin, count)` tuple of the trained user
    /// bigram via the public bigram export iterator.
    ///
    /// # Errors
    ///
    /// Returns an error if the iterator cannot be created, a step reports
    /// failure, or a string is not UTF-8.
    pub fn export_bigrams(&mut self) -> Result<Vec<ExportedPhrase>, OracleError> {
        // SAFETY: `self.context` is non-null and owned by `self` for the whole
        // call. A NULL iterator is checked below.
        let iter = unsafe { ffi::pinyin_begin_get_bigram_phrases(self.context) };

        if iter.is_null() {
            return Err(OracleError::Call {
                function: "pinyin_begin_get_bigram_phrases",
            });
        }
        ...
        let result = loop {
            // SAFETY: `iter` is non-null (checked above) and stays owned by
            // this call until `pinyin_end_get_bigram_phrases` below.
            let has_next = unsafe { ffi::pinyin_bigram_iterator_has_next_phrase(iter) };
            if !has_next {
                break Ok(());
            }
            ...
            // strings, released with `g_free` below exactly once each.
            let ok = unsafe {
                ffi::pinyin_bigram_iterator_get_next_phrase(
                    iter,
                    &raw mut phrase,
                    &raw mut pinyin,
                    &raw mut count,
            ...
            if !ok {
                break Err(OracleError::Call {
                    function: "pinyin_bigram_iterator_get_next_phrase",
                });
            }
        ...
        // SAFETY: `iter` is non-null and owned by this call; ended exactly
        // once on every path, and never used after.
        unsafe { ffi::pinyin_end_get_bigram_phrases(iter) };
