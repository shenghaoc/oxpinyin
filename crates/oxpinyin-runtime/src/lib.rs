...
pub struct RuntimeDict {
    ...
    /// The loaded-library mask: bit `n` clear = library `n` unloaded.
    /// Only `GBK_DICTIONARY` (2) is ever clearable — upstream's
...
impl RuntimeDict {
    ...
    /// Whether library `nibble` is visible (not unloaded).
    #[must_use]
    pub fn library_visible(&self, nibble: u32) -> bool {
        nibble != GBK_DICTIONARY
            || self.library_mask.load(Ordering::SeqCst) & (1 << GBK_DICTIONARY) == 0
    }
    ...
    pub fn add_unigram_delta(&self, token: u32, delta: u64) -> bool {
        ...
        let mut overlay = self
            ...
            .lock()
            .unwrap_or_else(|p| p.into_inner());
        *overlay.entry(token).or_insert(0) += delta;
        true
    }
    ...
    pub fn visible_item_count(&self) -> u64 {
        ...
        self.system
            .unigram_map()
            .keys()
            .filter(|&&token| mask & (1 << (token >> 24)) == 0)
            .count() as u64
    }
            ...
            1..=4 => {
                ...
                Some(TokenIntrospection {
                    ...
                    pronunciations: pronunciations
                        .into_iter()
                        .map(|(spelling, _freq)| {
                            let keys: Vec<PinyinKey> = spelling
                                .split('\'')
                                .map(|syllable| {
