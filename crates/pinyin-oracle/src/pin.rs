...
impl PinManifest {
    ...
    ///
    /// With `PINYIN_BENCH_DBM=kc|bdb` set, the pin-ref and DBM checks take the
    /// bench form instead — same libpinyin pin, DBM suffix per the env var —
    /// mirroring the link-time branch in `build.rs` so the two checks cannot
    /// disagree.
    ///
    /// # Errors
    ///
