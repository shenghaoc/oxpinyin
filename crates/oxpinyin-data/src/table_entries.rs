//! The two index DBMs' record builders — the write half of the
//! `ChewingLargeTable2` / `PhraseLargeTable3` tree tables
//! `chewing_table` / `phrase_table` read.
//!
//! The byte-level output of upstream's `ChewingLargeTable2::add_index` /
//! `PhraseLargeTable3::add_index` writer paths
//! (`docs/findings/pinyin-dbm-format-2026-09-01.md`,
//! `docs/findings/phrase-dbm-format-2026-09-01.md`): given semantic rows,
//! the exact `(key, value)` pairs those writers leave behind, prefix
//! markers included. `oxpinyin-datagen` writes the system tables with it;
//! the user store writes `user_pinyin_index.bin` /
//! `user_phrase_index.bin` with it — a user table entry passes through
//! the same `add_index` upstream, so the record shapes are identical.
//!
...
//! the container's physical order.
...
/// One row's parsed pronunciation, the input unit of both index writers.
