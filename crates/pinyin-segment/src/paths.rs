...
/// Environment variable naming the `pinyin-migrate export` directory.
...
pub struct SegmenterPaths {
    ...
    /// `bigram.redb` from `pinyin-migrate convert` / `export`.
    pub bigram: PathBuf,
        ...
        let export = locate_export_dir().ok_or_else(|| SegmentError::MissingPath {
            detail: format!(
                "no migrate export at ${EXPORT_DIR_ENV} or {DEFAULT_EXPORT_DIR}; \
                 run `pinyin-migrate export --out-dir {DEFAULT_EXPORT_DIR}`"
            ),
        })?;
...
fn workspace_root() -> Option<PathBuf> {
    ...
    loop {
        if dir.join("crates/pinyin-segment/Cargo.toml").is_file()
